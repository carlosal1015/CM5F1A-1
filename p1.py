...
def plot_bernstein(k: int = 3, n: int = 5):
    t = np.linspace(start=0, stop=1, num=100)
    plt.cla()
    plt.plot(t, bernstein(t=t, k=k, n=n), lw=0.45,
             label=f"$10t^5-20t^4+10t^3$")
    plt.grid()
...
def plot_curvature():
    t = np.linspace(start=0, stop=1, num=500)
    ...
    plt.grid()
    plt.legend(title="Leyenda", shadow=True, fancybox=True)
    plt.title(f"Función curvatura ${{\kappa}}(t)$ de $B_{{3, 5}}(t)$")
