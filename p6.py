...
plt.plot(x_new, Polynomial(
    poly.coef[::-1])(x_new), 'orange', lw=0.75, label=f"${{{p[2]}}}t^2{{{p[1]}}}t+{{{p[0]}}}$")
plt.grid()
plt.legend()
...
plt.cla()
plt.plot(x_new, 0.5*x_new*(x_new-1), lw=0.75, label=r"$\ell_{0}(t)=\frac{1}{2}t(t-1)$")
plt.plot(x_new, -(x_new+1)*(x_new-1), lw=0.75, label=r"$\ell_{1}(t)=-(t+1)(t-1)$")
plt.plot(x_new, 0.5*(x_new+1)*x_new, lw=0.75, label=r"$\ell_{2}(t)=\frac{1}{2}(t+1)t$")
plt.grid()
plt.legend()
